...
void buildSubTree(int round,int bet,int count,int totplayer,int player,int curParent,int curAction,double tot,double pc){
	...
	if (bet<round){
		...
		diff=bet+round-Bet[player];
		Util[player]-=diff;
		Bet[player]+=diff;
		buildSubTree(round,bet+round,1,totplayer,player%PlayerNum+1,curParent,1,tot+diff,pc);
		Util[player]+=diff;
		Bet[player]-=diff;
	}
